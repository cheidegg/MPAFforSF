...
SSDL2015::initialize(){
  ...
  string TR_lines_non_em[1] = {"HLT_BIT_HLT_Mu8_Ele8_CaloIdM_TrackIdM_Mass8_PFHT300_v"    };
  string TR_lines_non_mm[1] = {"HLT_BIT_HLT_DoubleMu8_Mass8_PFHT300_v"                    };

  string TR_lines_iso_ee[1] = {"HLT_BIT_HLT_Ele17_Ele12_CaloIdL_TrackIdL_IsoVL_DZ_v"      };  
  string TR_lines_iso_em[2] = {"HLT_BIT_HLT_Mu8_TrkIsoVVL_Ele17_CaloIdL_TrackIdL_IsoVL_v" , \
                               "HLT_BIT_HLT_Mu17_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL_v"};  
  string TR_lines_iso_mm[2] = {"HLT_BIT_HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ_v"            , \
                               "HLT_BIT_HLT_Mu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ_v"          };

  _vTR_lines_non_ee = Tools::toVector(TR_lines_non_ee);
  _vTR_lines_non_em = Tools::toVector(TR_lines_non_em);
  _vTR_lines_non_mm = Tools::toVector(TR_lines_non_mm);

  _vTR_lines_iso_ee = Tools::toVector(TR_lines_iso_ee);
  _vTR_lines_iso_em = Tools::toVector(TR_lines_iso_em);
  _vTR_lines_iso_mm = Tools::toVector(TR_lines_iso_mm);
...
SSDL2015::advancedSelection(int WF) {
  ...
  //CH: the HLT tree variables are not good, use the trigger bits instead 
...
SSDL2015::ssLeptonSelection() {
  ...
  if(_tightLepsOSPtCut.size()>=2) {
    ...
    int idvec[2] = {_idxL1, _idxL2};
	_auxIdxs.push_back( Tools::toVector(idvec));
      }
    }
  }
...
SSDL2015::chargeFlipProb() {
  ...
  for(unsigned int i=0;i<_auxPairs.size();i++) {
    ...
    _susyMod -> correctFlipRate(p1, _auxPairs[i][0]->eta());
    _susyMod -> correctFlipRate(p2, _auxPairs[i][1]->eta());
...
SSDL2015::selectLeptons() {
  ...
  _susyMod->cleanJets( &_jetCleanLeps10, _allJets, _allJetsIdx, _bJets, _bJetsIdx, 25, 25);
...
SSDL2015::registerTriggerVars(){
  for(unsigned int i = 0; i < _vTR_lines_non_ee.size(); ++i) _vc->registerVar(_vTR_lines_non_ee[i]);
  for(unsigned int i = 0; i < _vTR_lines_non_em.size(); ++i) _vc->registerVar(_vTR_lines_non_em[i]);
  for(unsigned int i = 0; i < _vTR_lines_non_mm.size(); ++i) _vc->registerVar(_vTR_lines_non_mm[i]);
  for(unsigned int i = 0; i < _vTR_lines_iso_ee.size(); ++i) _vc->registerVar(_vTR_lines_iso_ee[i]);
  for(unsigned int i = 0; i < _vTR_lines_iso_em.size(); ++i) _vc->registerVar(_vTR_lines_iso_em[i]);
  for(unsigned int i = 0; i < _vTR_lines_iso_mm.size(); ++i) _vc->registerVar(_vTR_lines_iso_mm[i]);
}
