...
Comissioning::defineOutput() {
 ...
 float bptvec[7] = {10,40,60,80,100,200,300}; 
 float etavec[4] = {0,0.8,1.479,2.5};
 vector<float> binsPt = Tools::toVector(bptvec);//lim sup mise par defaut
 vector<float> binsEta = Tools::toVector(etavec);//lim sup mise par defaut
 _binsPt = binsPt;
 _binsEta = binsEta;
  buildCategs();
...
Comissioning::buildCategs() {
  ...
  float ctvec[3] = {0,1,2};
  vector<float> ct = Tools::toVector(ctvec);
